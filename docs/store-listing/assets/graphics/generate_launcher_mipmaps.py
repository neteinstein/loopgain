"""
Regenerates the app's own legacy-fallback launcher icons — the static PNGs in
androidApp/src/main/res/mipmap-{mdpi,hdpi,xhdpi,xxhdpi,xxxhdpi}/ — from the same vector path
data as ic_launcher_foreground.xml / ic_launcher_background.xml, anti-aliased the same way as
generate_icon.py (4x supersample + LANCZOS downsample).

These PNGs are what pre-API26 launchers and some tooling fall back to; API26+ renders the
adaptive-icon vector XML directly and never touches these files, but they should still look as
good as the vector does. mipmap-anydpi-v26/*.xml is untouched — it needs no regeneration.

Run from anywhere:  python3 generate_launcher_mipmaps.py
"""
import os
import re
...
def draw_path(draw, path_d, grad, scale):
    stroke = 13.0 * scale
    ...
    for i in range(len(pts) - 1):
        ...
        draw.line(
            ...
            fill=c + (255,), width=round(stroke), joint="curve",
        )
        r = stroke / 2
        draw.ellipse([x0 * scale - r, y0 * scale - r, x0 * scale + r, y0 * scale + r], fill=c0 + (255,))
    xl, yl = pts[-1]
    cl = grad_color(xl, yl, grad)
    r = stroke / 2
    draw.ellipse([xl * scale - r, yl * scale - r, xl * scale + r, yl * scale + r], fill=cl + (255,))


def render_square(out_size):
    render = out_size * SS
    scale = render / VIEWPORT
    img = Image.new("RGBA", (render, render), BG)
    draw = ImageDraw.Draw(img)
    draw_path(draw, RIGHT_PATH, RIGHT_GRAD, scale)
    draw_path(draw, LEFT_PATH, LEFT_GRAD, scale)
    return img.resize((out_size, out_size), Image.LANCZOS)


def to_round(square_rgba):
    size = square_rgba.size[0]
    mask = Image.new("L", (size * SS, size * SS), 0)
    ImageDraw.Draw(mask).ellipse([0, 0, size * SS, size * SS], fill=255)
    mask = mask.resize((size, size), Image.LANCZOS)
    rounded = square_rgba.copy()
    rounded.putalpha(mask)
    return rounded


def main():
    res_dir = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..",
                                              "androidApp", "src", "main", "res"))
    for folder, size in DENSITIES.items():
        square = render_square(size)
        round_icon = to_round(square)
        out_dir = os.path.join(res_dir, folder)
        square.convert("RGB").save(os.path.join(out_dir, "ic_launcher.png"))
